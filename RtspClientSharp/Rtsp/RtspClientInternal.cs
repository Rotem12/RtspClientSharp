        ...
        private async Task SetupTrackAsync(RtspMediaTrackInfo track, CancellationToken token)
        {
            ...
            if (_connectionParameters.RtpTransport == RtpTransportProtocol.UDP)
            {
                ...
                try
                {
                    ...
                    int p = 0;
                    if (_connectionParameters.StreamUri != null)
                    {
                        p = _connectionParameters.StreamUri.Port;
                    }

                    IPEndPoint endPoint = new IPEndPoint(localIpToServer, p);
            ...
            else if (_connectionParameters.RtpTransport == RtpTransportProtocol.MULTICAST)
            {
                ...
                if(_connectionParameters.StreamUri != null)
                {
                    host = _connectionParameters.StreamUri.Host;
                    port = _connectionParameters.StreamUri.Port;
                }

                setupRequest = _requestMessageFactory.CreateSetupUdpMulticastRequest(track.TrackName, host, port);
                setupResponse = await _rtspTransportClient.EnsureExecuteRequest(setupRequest, token);
            }
            else
            {
