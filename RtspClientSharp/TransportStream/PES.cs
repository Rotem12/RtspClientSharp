...
namespace RtspClientSharp.Ts
{
    ...
    public class Pes
    {
        ...
        public Pes(TsPacket packet)
        {
            ...
            if (packet.PayloadLen > PesPacketLength)
            {
    //            Dropped = true;
    ...
    //            return;
            ...
            else //hopefully this is therefore video - so we'll stick a massive buffer in to catch
            {
                ...
                _data = ArrayPool<byte>.Shared.Rent(1024 * 1024);
                Buffer.BlockCopy(packet.Payload, 0, _data, _pesBytes, packet.PayloadLen);
                _pesBytes += (ushort)packet.PayloadLen;
            }
        ...
        public byte[] GetDataFromPes()
        {
            ...
            if (OptionalPesHeader != null && OptionalPesHeader.OptionalFields != null && OptionalPesHeader.OptionalFields.Length > 0)
            {
